...
def create_profile_hmms_and_consensus_seqs(
    ...
    try:
        ...
        for algn_file in _iter_progress(alignment_files, description="Creating HMMs and consensus"):
            ...
            # Create HMM using hmmbuild - restrict to a single CPU worker per job
            # because jobs are already parallelized across a multiprocessing pool.
            hmmbuild_cmd: List[str] = ['hmmbuild', '--cpu', '1', '--amino', hmm_path, algn_path, log_object]
...
def annotate_and_split_proteins_using_pfam(
    ...
    try:
        ...
        # Split the requested threads across the two levels of parallelism so their
        # product stays within `threads` - samples are processed concurrently by the
        # pool below and each of those jobs runs its own multithreaded hmmsearch.
        parallel_jobs = max(1, min(threads, len(sample_proteomes)))
        hmmsearch_cpus = max(1, threads // parallel_jobs)
        ...
        for sample_name, proteome_file in sample_proteomes.items():
            ...
            prot_mod_inputs.append([
                ...
                pfam_z, minimal_length, log_object, hmmsearch_cpus, skip_domain_splitting
            ])
        ...
        p = multiprocessing.Pool(parallel_jobs)
        for _ in _iter_progress(
