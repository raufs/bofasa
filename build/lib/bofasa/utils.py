...
def check_bofasa_db_setup(require_genomad: bool = False) -> Tuple[bool, str]:
    """
    Check that BOFASA_DB_PATH is set and points at a usable database directory.

    Validates the environment variable, the database_location_paths.txt manifest
    written by 'bofasa setup', and the ISfinder and Pfam entries needed by
    'bofasa prep'. The geNomad entry is only validated if requested.

    Parameters:
    -----------
    require_genomad : bool, default=False
        Whether the geNomad database entry is also required

    Returns:
    --------
    Tuple[bool, str]
        (is_available, message) - True if the databases are usable
    """
    bofasa_db_dir = os.getenv("BOFASA_DB_PATH", "").strip()
    if not bofasa_db_dir:
        return False, (
            "BOFASA_DB_PATH environment variable is not set.\n"
            "Please set it to the directory holding the bofasa annotation databases and\n"
            "run 'bofasa setup' if you have not done so already:\n"
            "  export BOFASA_DB_PATH=/path/to/databases"
        )

    if not os.path.isdir(bofasa_db_dir):
        return False, f"BOFASA_DB_PATH is set but is not a directory: {bofasa_db_dir}"

    db_locations = os.path.join(bofasa_db_dir, 'database_location_paths.txt')
    if not os.path.isfile(db_locations):
        return False, (
            f"Database locations file not found: {db_locations}\n"
            "Please run 'bofasa setup' to download and configure the annotation databases."
        )

    db_entries = {}
    try:
        with open(db_locations) as odb:
            for i, line in enumerate(odb):
                if i == 0:
                    continue
                ls = line.strip().split('\t')
        ...
        return False, f"Error reading database locations file {db_locations}: {str(e)}"

    required = ['isfinder', 'pfam']
    if require_genomad:
        required.append('genomad')

    for db_name in required:
        if db_name not in db_entries:
            return False, (
                f"No '{db_name}' entry in {db_locations}.\n"
                "Please run 'bofasa setup' to (re)configure the annotation databases."
            )
        db_path = db_entries[db_name][2]
        if not os.path.exists(db_path):
            return False, f"{db_name} database referenced in {db_locations} not found: {db_path}"

    pfam_entry = db_entries['pfam']
    if len(pfam_entry) < 4 or not pfam_entry[3].strip().isdigit():
        return False, (
            f"Pfam Z value missing or malformed in {db_locations}.\n"
            "Please run 'bofasa setup' to (re)configure the Pfam database."
        )

    return True, "bofasa databases are properly set up"
